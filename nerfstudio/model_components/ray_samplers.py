...
class ErrorBoundedSampler(Sampler):
    ...
    def generate_ray_samples(
        ...
        while not_converge and total_iters < self.max_total_iters:
            ...
            with torch.no_grad():
                ...
                new_deltas = new_samples.deltas.squeeze(-1)
            ...
            if sorted_index is not None:
                ...
                deltas_merge = torch.cat([deltas, new_deltas], -1)
                deltas = torch.gather(deltas_merge, 1, sorted_index)
            else:
                sdf = new_sdf
                deltas = new_deltas
            ...
            delta_density = deltas * density  # TODO shape does not match between deltas and density
    ...
    def get_dstar(self, sdf, ray_samples: RaySamples):
        ...
        d_star[first_cond] = b.float()[first_cond]
        d_star[second_cond] = c.float()[second_cond]
        s = (a + b + c) / 2.0
